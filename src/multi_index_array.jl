...
struct MultiIndexArray{
    T, N, AT <: AbstractArray{T, N}, AX <: NTuple{N, <: MultiIndexAxis}} <:
       AbstractMultiIndexArray{T, N}
    parent::AT
    axes::AX

    function MultiIndexArray(parent::AbstractArray{T, N}, axs::NTuple{
            N, MultiIndexAxis}) where {T, N}
        @assert all(length(axs[i]) == size(parent, i) for i in 1:N)
        new{T, N, typeof(parent), typeof(axs)}(parent, axs)
    end
end

function MultiIndexArray(parent::AbstractArray{T, N}, axs::Vararg{
        MultiIndexAxis, N}) where {T, N}
    MultiIndexArray(parent, axs)
end
function MultiIndexArray(parent::AbstractArray{T, N}, axs::Vararg{
        NTuple{M, Int}, N}) where {T, N, M}
    MultiIndexArray(parent, _to_multi_index_axes.(axs))
end
function MultiIndexArray(parent::AbstractArray{T, N},
        axs::Vararg{NTuple{M, AbstractUnitRange}, N}) where {T, N, M}
    MultiIndexArray(parent, _to_multi_index_axes.(axs))
end

function Base.:(==)(mia1::MultiIndexArray, mia2::MultiIndexArray)
...
function Base.getindex(mia::MultiIndexArray{T, N}, inds::Vararg{
